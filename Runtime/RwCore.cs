...
namespace BfbbImport
{
    ...
    public class RwReader : IDisposable
    {
        ...
        {
            byte[] buf = BR.ReadBytes(n);
            if (buf.Length < n)
                throw new System.IO.EndOfStreamException(
                    $"Expected to read {n} bytes at position {Base.Position - buf.Length}, " +
                    $"but only {buf.Length} bytes were available. The file may be truncated.");
            return buf;
        }
