    ...
    public static class DffParser
    {
        ...
        private static void ReadFrameList(RwReader r, long listEnd, RwClump clump)
        {
            ...
            for (int i = 0; i < numFrames && r.Position < listEnd; i++)
            {
                ...
                {
                    Debug.LogWarning($"[DffImporter] Frame {i} extension has unexpected chunk type 0x{eh.Type:X4} " +
                                      "(expected EXTENSION). Remaining frame extensions will be skipped.");
                    return;
                }
        ...
        private static RwGeometry ReadGeometry(RwReader r, long geomEnd)
        {
            ...
            if (r.Position < geomEnd)
            {
                ...
                {
                    Debug.LogWarning($"[DffImporter] Expected EXTENSION chunk after geometry material list, " +
                                      $"got 0x{eh.Type:X4}. Geometry extension data (e.g. skin) will be missing.");
                    r.Seek(peekPos);
                }
            }
        ...
        private static void ReadTextureChunk(RwReader r, long texEnd, RwMaterial mat)
        {
            ...
            if (nh.Type == RwChunk.STRING)
                ...
                Debug.LogWarning($"[DffImporter] Expected STRING chunk for texture name, got 0x{nh.Type:X4}. " +
                                  "Material will have no texture name.");
            r.Seek(nameEnd);

            if (r.Position < texEnd)
            {
                long maskEnd = r.ReadChunkHeader(out var mh2);
                if (mh2.Type == RwChunk.STRING)
                    mat.MaskName = r.ReadFixedString((int)mh2.Size);
                else
                    Debug.LogWarning($"[DffImporter] Expected STRING chunk for mask name, got 0x{mh2.Type:X4}. " +
                                      "Material will have no mask name.");
                r.Seek(maskEnd);
            }
        }
