...
namespace BfbbImport
{
    ...
    {
        // Safety limits for allocations driven by values read from untrusted binary files.
        // These are generous (well beyond any real BfBB model) while still preventing DoS
        // from crafted files that declare absurd counts.
        private const int MAX_FRAMES = 16384;
        private const int MAX_GEOMETRIES = 16384;
        private const int MAX_VERTICES = 2_000_000;
        private const int MAX_TRIANGLES = 4_000_000;
        private const int MAX_MATERIALS = 4096;
        private const int MAX_BONES = 512;
        ...
        private static void ReadFrameList(RwReader r, long listEnd, RwClump clump)
        {
            ...
            if (numFrames < 0 || numFrames > MAX_FRAMES)
                throw new InvalidDataException($"FRAME_LIST declares {numFrames} frames, exceeds safe limit ({MAX_FRAMES}).");
        ...
        private static void ReadGeometryList(RwReader r, long listEnd, RwClump clump)
        {
            ...
            if (numGeometries < 0 || numGeometries > MAX_GEOMETRIES)
                throw new InvalidDataException($"GEOMETRY_LIST declares {numGeometries} geometries, exceeds safe limit ({MAX_GEOMETRIES}).");
            r.Seek(structEnd);
        ...
        private static RwGeometry ReadGeometry(RwReader r, long geomEnd)
        {
            ...
            if (numVertices < 0 || numVertices > MAX_VERTICES)
                throw new InvalidDataException($"Geometry declares {numVertices} vertices, exceeds safe limit ({MAX_VERTICES}).");
            if (numTriangles < 0 || numTriangles > MAX_TRIANGLES)
                throw new InvalidDataException($"Geometry declares {numTriangles} triangles, exceeds safe limit ({MAX_TRIANGLES}).");
        ...
        private static void ReadMaterialList(RwReader r, long listEnd, RwGeometry g)
        {
            ...
            if (numMaterials < 0 || numMaterials > MAX_MATERIALS)
                throw new InvalidDataException($"MATERIAL_LIST declares {numMaterials} materials, exceeds safe limit ({MAX_MATERIALS}).");
        ...
        private static RwSkin ReadSkin(RwReader r, int numVertices)
        {
            ...
            if (numBones > MAX_BONES)
                throw new InvalidDataException($"SKIN_PLG declares {numBones} bones, exceeds safe limit ({MAX_BONES}).");
