...
namespace BfbbImport.Editor
{
    ...
    public static class DffBatchImport
    {
        ...
        private static void BatchImportExternalFolder()
        {
            ...
            int copied = 0;
            string destFolderFull = Path.GetFullPath(destFolder).Replace('\\', '/');
            foreach (var srcFile in sourceFiles)
            {
                string relative = Path.GetRelativePath(sourceFolder, srcFile);
                string destFile = Path.GetFullPath(Path.Combine(destFolder, relative));

                // Prevent path traversal: ensure the resolved destination stays inside the
                // user-chosen target folder. A malicious folder tree with ".." segments
                // could otherwise write files outside the intended project location.
                if (!destFile.Replace('\\', '/').StartsWith(destFolderFull))
                {
                    Debug.LogWarning($"[DffImporter] Skipping '{relative}' — resolved path escapes the destination folder (possible path traversal).");
                    continue;
                }
